...
from localstack.utils.analytics.metrics import (
    Counter, MockCounter, get_metric_registry
)
...
def test_counter_increment(counter):
    ...
    assert collected[0]["value"] == 4, f"Unexpected counter value: expected 4, got {collected[0]['value']}"
...
def test_counter_reset(counter):
    ...
    counter.reset()
    collected = counter.collect()
    assert collected[0]["value"] == 0, f"Unexpected counter value: expected 0, got {collected[0]['value']}"
...
def test_labeled_counter_increment(labeled_counter):
    ...
    assert any(metric["value"] == 2 for metric in collected if
               metric["label_1"] == "success"), f"Unexpected counter value for label success"
    assert any(metric["value"] == 3 for metric in collected if
               metric["label_1"] == "error"), f"Unexpected counter value for label error"
...
def test_labeled_counter_reset(labeled_counter):
    ...
    collected = labeled_counter.collect()
    assert any(metric["value"] == 0 for metric in collected if
               metric["label_1"] == "success"), f"Unexpected counter value for label success"

    assert any(metric["value"] == 4 for metric in collected if
               metric["label_1"] == "error"), f"Unexpected counter value for label error"
...
def test_metric_registry_register_and_collect(counter):
    ...
    assert any(
        metric["value"] == 7 for metric in collected["metrics"]
...
def test_thread_safety(counter):
    ...
    collected = counter.collect()
    assert collected[0]["value"] == 5000, f"Unexpected counter value: expected 5000, got {collected[0]['value']}"
